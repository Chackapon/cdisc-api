// TAG DISC
// Important constants
import { MOD_NAMESPACE, JUKEBOX_DATA_NAME } from "./default_const";
// Dynamic world data handler library
import { loadWorldData } from './world_data_save.js';
//
import { jukeboxRegistry, currentPlayingRegistry, } from "./registries";
    ...
    system.runInterval(() => {
        currentPlayingRegistry.forEach((duration, jukeboxID) => {
            if (duration - 1 <= 0) {
                ...
                currentPlayingRegistry.set(jukeboxID, duration - 1);
        });
    }, 20);
}
    ...
    system.runInterval(() => {
        currentPlayingRegistry.forEach((value, jukeboxID) => {
            ...
            if (jukeboxData) {
                ...
                dimension.spawnParticle("minecraft:note_particle", {
                    x: location.x + 0.5,
                    y: location.y + 1.2,
                    z: location.z + 0.5
                });
